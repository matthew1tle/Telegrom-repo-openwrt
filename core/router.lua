...
local function send_main_menu(chat_id, message_id)
    ...
    kb_engine.add_row(kb, {
        { text = "📶 Wi-Fi Settings", callback_data = "nav_wifi" },
        { text = "🌐 WAN / Internet", callback_data = "nav_internet" }
    })
    kb_engine.add_row(kb, {
        { text = "👥 Clients List", callback_data = "nav_clients" },
        { text = "📦 Packages", callback_data = "nav_pkg" }
    })
    kb_engine.add_row(kb, {
        { text = "🧱 Passwall", callback_data = "nav_passwall" },
        { text = "⚡ Sing-box", callback_data = "nav_singbox" }
    })
    kb_engine.add_row(kb, {
        { text = "💻 System Specs", callback_data = "nav_system" },
        { text = "📊 Live Monitor", callback_data = "nav_monitor" }
    })
    ...
    if message_id then
        telegram.edit_message(chat_id, message_id, "🎮 *OpenWrt Router Control Board*", markup)
    else
        telegram.send_message(chat_id, "🎮 *OpenWrt Router Control Board*", markup)
    end
end

-- Processes Text Messages Natively (Captures user text entries for passwords/SSIDs)
...
function M.handle_callback(callback)
    ...
    elseif data == "nav_wifi" then
        ...
        for _, net in ipairs(nets) do
            -- Submenu button row creation per active radio interface
            kb_engine.add_row(kb, {
    ...
    elseif data:match("^wifi_reqssid_") then
        ...
        telegram.edit_message(chat_id, message_id, "📝 *Type the new Wi-Fi name (SSID)* and send it as a text message now:", kb_engine.export(kb_engine.build_confirmation("nil", "nav_wifi")))
        telegram.answer_callback(query_id)
    ...
    elseif data:match("^wifi_reqpass_") then
        ...
        telegram.edit_message(chat_id, message_id, "🔑 *Type the new Wi-Fi Password* (minimum 8 characters) and send it as a text message now:", kb_engine.export(kb_engine.build_confirmation("nil", "nav_wifi")))
        telegram.answer_callback(query_id)

    elseif data == "nav_internet" then
        local text = internet.get_internet_summary()
        local kb = kb_engine.create()
        kb_engine.add_row(kb, { { text = "🚀 Run Speed Test", callback_data = "net_speedtest" } })
        kb_engine.add_control_row(kb, "nav_main", "nav_internet")
        telegram.edit_message(chat_id, message_id, text, kb_engine.export(kb))
        telegram.answer_callback(query_id)

    elseif data == "net_speedtest" then
        telegram.answer_callback(query_id, "Running speed metrics over Cloudflare Backbone network...", false)
    ...
    elseif data == "nav_monitor" then
        telegram.answer_callback(query_id, "Calculating bandwidth speed delta statistics...", false)
        local rx, tx = monitor.get_realtime_stats()
        local text = string.format("📊 *Realtime Network Stats*\n\n📥 Download: `%s/s`\n📤 Upload: `%s/s`", helpers.format_bytes(rx), helpers.format_bytes(tx))
        local kb = kb_engine.create()
        kb_engine.add_control_row(kb, "nav_main", "nav_monitor")
        telegram.edit_message(chat_id, message_id, text, kb_engine.export(kb))
    else
        telegram.answer_callback(query_id, "Action parameter unrecognized.", true)
    end
end
